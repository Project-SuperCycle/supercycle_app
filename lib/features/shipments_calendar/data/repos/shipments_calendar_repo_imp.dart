...
class ShipmentsCalendarRepoImp implements ShipmentsCalendarRepo {
  ...
  Future<Either<Failure, List<ShipmentModel>>> getAllTraderDoneShipments({
    ...
    try {
      final response = await apiServices.get(
        endPoint: ApiEndpoints.getAllShipments,
        query: {"page": page, "status": "delivered"},
      );
  ...
  Future<Either<Failure, List<ShipmentModel>>> getAllRepDoneShipments({
    ...
    try {
      ...
      Logger().i("shipments ${shipments.length}");
      List<ShipmentModel> doneShipments = shipments.where((shipment) {
        final status = shipment.status.trim();
        return status == 'تم الاستلام';
      }).toList();
      Logger().i("doneShipments ${doneShipments.length}");
      return right(shipments);
    } on DioException catch (dioError) {
      return left(ServerFailure.fromDioError(dioError));
    } on FormatException catch (formatError) {
